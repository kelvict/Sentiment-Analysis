...
if __name__ == '__main__':
    ...
    for j in range(1, 2):
        ...
        for item in ['t1','t2','t3','t4','t5']:
            ...
            class_dict = {'1': 'against','2': 'favor', '3': 'none'}

            classifier_list_plus = ['lg', 'nb', 'svm', 'mixed']
            with open(data_dir + os.sep + 'result.txt', 'a') as xs:
                xs.write(item + '\n')
                for c in classifier_list_plus:
                    result_dict = performance.demo_cv_performance(output_dir, fold_num, class_dict, c)
                    ss = ''
                    for key in ['p_against','r_against','f1_against',
                    'p_favor','r_favor','f1_favor','p_none','r_none','f1_none','macro_f1','acc']:
                        ss += str(round(result_dict[key]*100,4))+'%\t'
                    print ss.rstrip('\t')
                    xs.write("classifier " + c + '\n')
                    xs.write(ss.rstrip('\t') + '\n')
